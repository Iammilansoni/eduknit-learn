...
async function finalSetup() {
  ...
  try {
    ...
    console.log('📚 Setting up all courses...\n');
    ...
    for (const courseData of requiredCourses) {
      ...
      console.log(`  ✅ Created: ${course.title} (${course.slug})`);
    }
    
    // Create sample modules and lessons for Communication Skills
    const commSkillsCourse = createdCourses.find(c => c.slug === 'communication-skills');
    if (commSkillsCourse) {
      console.log('\n📖 Creating modules and lessons for Communication Skills...');
      ...
      for (let moduleIndex = 0; moduleIndex < modules.length; moduleIndex++) {
        ...
        const module = new ProgrammeModule({
          programmeId: commSkillsCourse._id,
    ...
    // Final verification
    const finalCourses = await Programme.find({}).select('title slug _id');
    const finalMapping = {};
    ...
    finalCourses.forEach((course, index) => {
      ...
      finalMapping[course.slug] = course._id.toString();
      console.log('');
    });
    
    console.log('🔗 Course Mapping (for API):');
    console.log(JSON.stringify(finalMapping, null, 2));
    
    console.log('\n✅ LMS System Ready!');
    console.log('✅ All 6 courses are available for enrollment');
    console.log('✅ Communication Skills has full content structure');
    console.log('✅ Backend API endpoints are ready');
    console.log('✅ Frontend can now enroll in all courses');
    ...
    console.log('   POST /api/course/enroll    - Enroll in course');
