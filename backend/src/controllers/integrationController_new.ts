...
export const sendTestNotification = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (platform === 'discord') {
      ...
      const message = 'This is a test notification from EduKnit Learning Platform. If you received this, your Discord integration is working correctly!';

      await discordService.sendAnnouncement(userId.toString(), title, message);
      success(res, null, 'Test notification sent successfully');
    } else {
      error(res, 'Platform not supported for notifications yet', 400, 'NOT_IMPLEMENTED');
    }
  } catch (err) {
