...
router.post('/courses', [
  ...
  body('totalModules').optional().isInt({ min: 1 }).withMessage('Total modules must be at least 1'),
  body('totalLessons').optional().isInt({ min: 1 }).withMessage('Total lessons must be at least 1'),
  body('estimatedDuration').optional().isFloat({ min: 1 }).withMessage('Estimated duration must be at least 1 hour'),
