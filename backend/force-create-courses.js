...
console.log('Starting course creation script...');
...
async function forceCreateCourses() {
  try {
    console.log('Connecting to MongoDB...');
    await mongoose.connect(MONGODB_URI);
    console.log('Connected to MongoDB successfully!');

    console.log(`\nProcessing ${requiredCourses.length} courses...`);

    for (let i = 0; i < requiredCourses.length; i++) {
      const courseData = requiredCourses[i];
      console.log(`\n[${i + 1}/${requiredCourses.length}] Processing: ${courseData.title}`);
      
      try {
        // Force create/update the course
        ...
        console.log(`✅ SUCCESS: ${result.title} (${result.slug}) [${result._id}]`);
      } catch (error) {
        console.log(`❌ ERROR with ${courseData.title}:`, error.message);
      }
    }
    ...
    console.log(`Total active courses in database: ${allCourses.length}`);
    ...
    console.log('\n✅ Course creation script completed successfully!');
